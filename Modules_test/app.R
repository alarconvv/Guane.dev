...
server <- function(input, output,session) {
  ...
  # Info vector
  infoServer("infoDisCharDT", info = VarDisCharDT$info)
  ...
  StatesDisChar <- eventReactive(input$importCSVDisCharDT,{
    if (input$ExCVSDisCharDT == T){
      readRDS(file = 'examples/anoleData.RDS')
    }else{
  ...
  observeEvent(input$importCSVDisCharDT, {
    VarDisCharDT$info <- StatesDisChar()
  })
